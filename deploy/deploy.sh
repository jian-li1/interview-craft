#!/usr/bin/env bash
# InterviewBlueprint — one-command deploy to Google Cloud Run.
#
...
deploy_backend() {
  ...
  gcloud run deploy interview-blueprint-backend \
  ...
  BACKEND_URL=$(gcloud run services describe interview-blueprint-backend \
    --project "$PROJECT_ID" --region "$REGION" --format 'value(status.url)')
  echo "Backend: $BACKEND_URL"
  echo "NOTE: set FRONTEND_ORIGIN on the backend after the frontend deploy:"
  echo "  gcloud run services update interview-blueprint-backend --region $REGION --update-env-vars FRONTEND_ORIGIN=<frontend-url>"
}
...
deploy_frontend() {
  ...
  gcloud builds submit "$ROOT/frontend" \
    --project "$PROJECT_ID" \
    --tag "$REGION-docker.pkg.dev/$PROJECT_ID/interview-blueprint/frontend:latest" 2>/dev/null || {
      echo "Artifact Registry repo missing — creating it..."
      gcloud artifacts repositories create interview-blueprint \
  ...
  gcloud run deploy interview-blueprint-frontend \
    --project "$PROJECT_ID" --region "$REGION" \
    --image "$REGION-docker.pkg.dev/$PROJECT_ID/interview-blueprint/frontend:latest" \
    --allow-unauthenticated --port 3000 \
    --memory 512Mi --min-instances 0 --max-instances 4
  FRONTEND_URL=$(gcloud run services describe interview-blueprint-frontend \
    --project "$PROJECT_ID" --region "$REGION" --format 'value(status.url)')
  echo "Frontend: $FRONTEND_URL"
  echo "==> Pointing backend CORS at the frontend..."
  gcloud run services update interview-blueprint-backend \
    --project "$PROJECT_ID" --region "$REGION" \
